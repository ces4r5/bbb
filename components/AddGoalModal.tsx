...
import { X, Plus, Calendar, Clock, Target } from 'lucide-react-native';
...
export function AddGoalModal({ visible, onClose, subjects, onSave }: AddGoalModalProps) {
  const [selectedSubject, setSelectedSubject] = useState('');
  const [totalHours, setTotalHours] = useState('');
  const [distributionType, setDistributionType] = useState('uniform'); // 'uniform' or 'custom'
  ...
  const calculateUniformDistribution = () => {
    const enabledDays = Object.keys(schedule).filter(day => schedule[day].enabled);
    const hoursPerDay = parseFloat(totalHours) / enabledDays.length;
    
    const newSchedule = { ...schedule };
    enabledDays.forEach(day => {
      newSchedule[day].hours = hoursPerDay;
    });
  ...
  const handleSave = () => {
    ...
    const scheduledHours = Object.values(schedule).reduce((total, day) => {
    ...
    const goal = {
      ...
      distributionType,
      schedule: distributionType === 'uniform' ? 
        (() => {
          const newSchedule = { ...schedule };
          ...
          <ScrollView style={styles.content}>
            ...
            <View style={styles.summary}>
              ...
              <Text style={styles.summaryText}>
                Total: {totalHours || '0'}h • {Object.keys(schedule).filter(day => schedule[day].enabled).length} dias
              </Text>
            </View>
          </ScrollView>
